            ...
            >
                ...
                <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10 rounded-t-2xl">
                    ...
                    <div className="flex items-center gap-2 ml-3">
                        <button
                            ...
                            className={`p-2 rounded-lg transition-colors ${editMode ? "bg-indigo-100 text-indigo-600" : "hover:bg-gray-100 text-gray-500"}`}
                        >
                            <Edit2 size={16} />
                        </button>
                        <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 md:hidden">
                            <X size={18} />
                        </button>
                        <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 hidden md:block">
                            <X size={18} />
                        </button>
                    </div>
                </div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    {card.status === "DONE" && stars && (
                        <div className="bg-amber-50 rounded-xl p-4 text-center border border-amber-100">
                            <p className="text-sm text-amber-700 font-medium mb-1">¡Tema completado! 🎉</p>
                            <p className="text-2xl">{stars}</p>
                            <p className="text-xs text-amber-600 mt-1">{Math.floor(totalSeconds / 3600)} hora{Math.floor(totalSeconds / 3600) !== 1 ? "s" : ""} de estudio</p>
                        </div>
                    )}
                    ...
                    {card.status !== "DONE" && (
                        <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                            <h3 className="text-sm font-semibold text-gray-700 mb-4">⏱ Cronómetro Pomodoro</h3>
                            <PomodoroTimer
                                cardId={card.id}
                                ...
                                onStatusChange={() => onUpdate(card.id, { status: "DOING" } as any)}
                            />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            {editMode ? (
                                ...
                                <p className="text-sm text-gray-800 font-medium">
                                    {studyType === "VIDEO" ? "🎬 Video" : studyType === "AUDIO" ? "🎧 Audio" : "📖 Texto"}
                                </p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            {editMode ? (
                                ...
                                <p className="text-sm text-gray-800 font-medium">
                                    {isVideo
                                        ? currentMinute > 0 ? `Min ${currentMinute}` : "No registrado"
                                        : currentPage > 0 ? `Pág. ${currentPage}` : "No registrado"}
                                </p>
                            )}
                        </div>
                            ...
                            {editMode ? (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        />
                                        <button type="button" onClick={handleCreateCategory} className="px-2 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-medium">+</button>
                                    </div>
                                </div>
                            ) : (
                                <p className="text-sm text-gray-800 font-medium">
                                    {card.category ? (
                                        <span className="inline-flex items-center gap-1">
                                            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: card.category.color }} />
                                            {card.category.name}
                                        </span>
                                    ) : "Sin categoría"}
                                </p>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            className="flex-1 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                        />
                                        <button type="button" onClick={handleCreateSeries} className="px-2 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-medium">+</button>
                                    </div>
                                </div>
                            ) : (
                                <p className="text-sm text-gray-800 font-medium">
                ...
                >
                    ...
                    <div className="sm:col-span-2">
                        ...
                        {editMode ? (
                            ...
                            <p className="text-sm text-gray-800 font-medium">{source || "No registrada"}</p>
                        )}
                    </div>
                ...
                <div className="bg-gray-50 rounded-xl p-4 grid grid-cols-2 gap-3 text-xs border border-gray-100">
                    <div>
                        ...
                        <p className="font-medium text-gray-800">{new Date(card.createdAt).toLocaleString("es-CO")}</p>
                    </div>
                    {card.startedAt && (
                        <div>
                            <p className="text-gray-500">Iniciado</p>
                            <p className="font-medium text-gray-800">{new Date(card.startedAt).toLocaleString("es-CO")}</p>
                        </div>
                    )}
                    {card.lastActiveAt && (
                        <div>
                            <p className="text-gray-500">Último avance</p>
                            <p className="font-medium text-gray-800">{new Date(card.lastActiveAt).toLocaleString("es-CO")}</p>
                        </div>
                    )}
                    {card.completedAt && (
                        <div>
                            <p className="text-gray-500">Completado</p>
                            <p className="font-medium text-gray-800">{new Date(card.completedAt).toLocaleString("es-CO")}</p>
                        </div>
                    )}
                </div>
