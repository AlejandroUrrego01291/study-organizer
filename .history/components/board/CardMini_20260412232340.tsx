...
function getInactivityColor(card: StudyCard): string {
    ...
    if (card.status === "DOING") {
        const lastActive = card.lastActiveAt
            ? new Date(card.lastActiveAt)
...
export default function CardMini({ card, onClick, onDelete }: Props) {
    ...
    return (
        <div
            className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden cursor-pointer hover:shadow-md transition-shadow group w-full max-w-full"
            onClick={onClick}
        >
                ...
                <div className="flex items-start justify-between gap-2 min-w-0">
                    <div className="flex-1 min-w-0 overflow-hidden">
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            {card.category && (
                                <span
                                    className="text-xs px-1.5 py-0.5 rounded-full text-white truncate max-w-[100px]"
                        ...
                        {card.status === "DONE" && card.totalStudySeconds > 0 && (
                            <p className="text-xs mt-1 truncate">{getStars(card.totalStudySeconds)}</p>
                        )}
