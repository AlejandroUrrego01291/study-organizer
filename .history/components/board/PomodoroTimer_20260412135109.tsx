...
export default function PomodoroTimer({
    ...
    // Hook para advertir al usuario si intenta salir
    ...
    function handleTimerEnd(
        ...
        if (currentPhase === "focus" || currentPhase === "idle") {
            ...
            if (isLongBreak) {
                const message = "🛋️ ¡Completaste 4 pomodoros! Mereces un descanso largo."
                toast(message, { duration: 8000 })
                sendNotification("🛋️ Descanso Largo", "¡Excelente trabajo! Toma tu descanso largo.")
                setPhase("long_break")
                setSecondsLeft(longBreakMinutes * 60)
            } else {
                const message = "☕ ¡Buen trabajo! Toma un descanso de 5 minutos."
                toast(message, { icon: "🍅", duration: 6000 })
                sendNotification("🍅 Pomodoro Completado", "¡Buen trabajo! Toma un descanso corto.")
                setPhase("short_break")
                setSecondsLeft(shortBreakMinutes * 60)
            }
        } else {
            toast("🎯 ¡Descanso terminado! A enfocarse de nuevo.", { duration: 4000 })
            sendNotification("🎯 ¡Listo para enfocarte!", "El descanso ha terminado.")
            setPhase("focus")
            setSecondsLeft(focusMinutes * 60)
            setIsRunning(false)
        }
